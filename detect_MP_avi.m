...
img_nt = 0;
imgset_cat = cell(size(imgset));
for j = 1:length(imgset)
    imgset_cat{j} = VideoReader([dir_name imgset(j).name]);
    img_nt = img_nt + imgset_cat{j}.NumFrames;
end
...
%     idx = cal_dots.Area > 300 & cal_dots.Area < 5000;
    ...
    % preload avi files
    start_idx = ones(size(imgset));
    vid = cell(size(imgset));
    file_id = uint8(zeros(img_nt,1));
    for j = 1:length(imgset)
        vid{j} = VideoReader([dir_name imgset(j).name]);
        file_id(start_idx(j):(start_idx(j) + vid{j}.NumFrames - 1)) = j;
        if j < length(imgset)
            start_idx(j+1) = start_idx(j) + vid{j}.NumFrames;
        end
    end

    parfor i = i0:i0+nframes-1 % parfor

        A = cam_aviread(vid{file_id(i)}, i-start_idx(file_id(i))+1, cam_left);
...
if save_on
    save(sprintf('%sMP in OSBL\\imaging expts\\run%s\\outputs\\tracks_run%02d.mat', gdrive_path, expt_string, run_params.Run(n)),'tracks0','tracklength0')
end
